...
import { DEPARTMENTS, SDG_GOALS, SCHOOL_LEVELS, isUserCasCoordinator, CAS_DEPARTMENT_ID } from '../../constants';
...
export const ProjectFormModal: React.FC<ProjectFormModalProps> = ({
  ...
  const isCasCoord = isUserCasCoordinator(currentUser);
  const isDeptLocked = (currentUser.role === 'teacher' || currentUser.role === 'dept_head') && !isCasCoord;
  ...
  const availableDepartments = useMemo(() => {
    ...
    if (isCasCoord) {
      ...
      const casDept = DEPARTMENTS.find(d => d.id === CAS_DEPARTMENT_ID);
      const list = [];
      if (userDept) list.push(userDept);
      if (casDept && casDept.id !== userDept?.id) list.push(casDept);
      return list.length > 0 ? list : DEPARTMENTS;
    }
    if (currentUser.role === 'teacher' || currentUser.role === 'dept_head') {
      const userDept = DEPARTMENTS.find(d => d.id === currentUser.departmentId);
      return userDept ? [userDept] : DEPARTMENTS;
    }
    return DEPARTMENTS;
  }, [currentUser, isCasCoord]);
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                ...
                {isCasCoord ? (
                  <span className="text-[10px] font-bold text-amber-800 bg-amber-50 border border-amber-200 px-2 py-0.5 rounded flex items-center gap-1">
                    ...
                    IB CAS Koordinatörü (Zümreniz veya CAS)
                  </span>
                ) : isDeptLocked ? (
              ...
              >
                {availableDepartments.map((dept) => (
                  <option key={dept.id} value={dept.id}>
                    {dept.id === CAS_DEPARTMENT_ID ? `★ ${dept.name} (IB DP)` : dept.name}
                  </option>
                ))}
              </select>
              ...
              {isCasCoord && (
                <div className="mt-2 text-xs">
                  {departmentId === CAS_DEPARTMENT_ID ? (
                    <div className="p-2.5 rounded-xl bg-amber-50/90 border border-amber-200 text-amber-900 flex items-start gap-2">
                      <Sparkles className="w-3.5 h-3.5 text-amber-600 shrink-0 mt-0.5" />
                      <div>
                        <span className="font-bold block text-[11px] text-amber-950">CAS Proje Başvurusu</span>
                        <span className="text-[11px] text-amber-800 leading-relaxed">
                          Bu proje <strong>CAS (Creativity, Activity, Service)</strong> kapsamında önerilecek ve ön onay için doğrudan <strong>IB DP Koordinatörüne</strong> sevk edilecektir.
                        </span>
                      </div>
                    </div>
                  ) : (
                    <div className="p-2.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-700 flex items-start gap-2">
                      ...
                      <div>
                        <span className="font-bold block text-[11px] text-slate-900">Branş Zümresi Başvurusu</span>
                        <span className="text-[11px] text-slate-600 leading-relaxed">
                          Bu proje branş zümreniz kapsamında önerilecek ve değerlendirme için <strong>Bölüm Başkanınıza</strong> iletilecektir.
                        </span>
                      </div>
                    </div>
                  )}
